...
static esp_err_t system_init(void)
{
    ...
    // DISABLED: No display, no UI needed
    ESP_LOGW(TAG, "UI manager initialization SKIPPED (no display)");
    xEventGroupSetBits(g_system_event_group, UI_READY_BIT);  // Set bit to avoid blocking
...
static esp_err_t start_application_tasks(void)
{
    ...
    // DISABLED: No display/UI
    ESP_LOGW(TAG, "UI task SKIPPED (no display)");

    // Start audio tasks (recording and playback)
    esp_err_t ret = audio_pipeline_start_tasks();
    if (ret != ESP_OK) {
