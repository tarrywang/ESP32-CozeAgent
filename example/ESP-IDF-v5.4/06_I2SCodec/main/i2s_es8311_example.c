...
{
    while (1) {
        status_display_set_audio_status(&g_audio_status);
...
static void i2s_music(void *args)
{
    ...
    // Update audio status
    g_audio_status.is_playing = true;
    ...
    while (1) {
        ...
        if (bytes_write > 0) {
            ESP_LOGI(TAG, "[music] i2s music played, %zu bytes are written.", bytes_write);
        } else {
...
static void i2s_echo(void *args)
{
    ...
    // Update audio status
    g_audio_status.is_recording = true;
...
void app_main(void)
{
    ...
    if (status_display_init() != ESP_OK) {
        ESP_LOGE(TAG, "Display init failed");
        // Continue without display
    } else {
        ESP_LOGI(TAG, "Display init success");
        // Start display update task
        xTaskCreate(display_update_task, "disp_update", 4096, NULL, 3, NULL);
    }
