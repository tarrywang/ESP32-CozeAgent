...
static void bsp_lvgl_rounder_cb(lv_disp_drv_t *disp_drv, lv_area_t *area)
{
    ...
    // Clamp to max valid coordinate to prevent overflow when gap is added
    // Hardware range: x_gap(6) to (x_gap + resolution - 1) = 6 to 471
    // After adding gap, max coordinate must be: resolution - 1 = 465
    // 465 + gap(6) = 471 (exactly fits hardware max)
